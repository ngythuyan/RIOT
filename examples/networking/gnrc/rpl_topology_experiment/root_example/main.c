...
int main(void)
{
    ...
    static char server_buffer[sizeof(msg_ping_t) + 10];
    int num = 0;
    ...
    while (1) {
        ...
        if ((res = sock_udp_recv(&sock, server_buffer,
                                sizeof(msg_ping_t), SOCK_NO_TIMEOUT,
                                &remote)) < 0) {
        ...
        for (int i = 0; i < num; i++) {
            if (nodes[i].here > 0 && strncmp(nodes[i].child, sender, 5) == 0) {
                ...
                int cmp = strncmp(nodes[i].parent, ping->parent, 5);
                uint32_t time_now = ztimer_now(ZTIMER_MSEC) - time_start;
                printf("Node:%ld;%s;%s;%ld;%ld;%d;%d;%d\n", time_now, sender, ping->parent, 
                nodes[i].parent_change, ping->msg_no, ping->hp, ping->lqi, ping->rssi);
                if (cmp != 0) {
                    nodes[i].parent_change++;
                    strncpy(nodes[i].parent, ping->parent, 5);
                    nodes[i].parent[4] = '\0';
                    print_tree(nodes);
                }
                found = true;
        ...
        strncpy(nodes[num].parent, ping->parent, 5);
        num++;
        print_tree(nodes);
        printf("Time:%ld-NUM:%d\n", ztimer_now(ZTIMER_MSEC) - time_start, num);
    }
    return 0;
}
