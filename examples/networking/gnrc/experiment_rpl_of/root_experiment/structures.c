...
uint8_t put_node(ipv6_addr_t addr, uint32_t replies, char parent[], node_info nodes[])
{
    ...
    if(target_slot->occupied == false)
    {
        ...
        strncpy(target_slot->current_parent.parent, parent, IPV6_CUSTOM_ADDR_STR_LEN - 1);
        target_slot->current_parent.parent[IPV6_CUSTOM_ADDR_STR_LEN - 1] = '\0';
        return 1;
    }
    else if (strncmp(target_slot->current_parent.parent, parent, IPV6_CUSTOM_ADDR_STR_LEN) != 0)
    {
